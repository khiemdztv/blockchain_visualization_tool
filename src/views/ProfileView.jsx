              ...
              <div key={c._id} className="card" style={{ marginBottom: 12, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
                ...
                <button className="btn btn-ghost btn-sm" onClick={() => {
                  import('jspdf').then(({ jsPDF }) => {
                    ...
                    doc.text(c.displayName || 'Student', 148.5, 118, { align: 'center' });
                    doc.setFontSize(12); doc.setTextColor(148, 163, 184);
                    doc.text(`Score: ${c.score}/${c.totalQuestions}`, 148.5, 140, { align: 'center' });
                    doc.setFontSize(10); doc.setTextColor(100, 116, 139);
                    doc.text(`Code: ${c.certCode} • ${new Date(c.issuedAt).toLocaleDateString()}`, 148.5, 170, { align: 'center' });
                    doc.save(`HubBlock_${c.certCode}.pdf`);
                  });
                }}>
