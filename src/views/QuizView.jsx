...
function drawHexPattern(doc) {
  // light hex pattern left and right like the reference image
  const leftX = 36;
  const rightX = 297 - 36;
  const baseY = 108;
...
function drawHubBlockLogo(doc, x, y) {
  // Simple cube/hex mark + HUBBLOCK text (vector, no external asset)
  // icon
...
function drawCertifiedStamp(doc, cx, cy) {
  // Approximate red "CERTIFIED" stamp
...
async function generateCertPDF(certificate) {
  try {
    ...
    // Corner ribbons + subtle patterns
    drawCornerRibbons(doc);
    drawHexPattern(doc);

    // Top area: logo + HUBBLOCK EDUCATION
    ...
    // Big CERTIFICATE title (serif-like)
    ...
    // Name (script-ish using Times Italic)
    doc.setFont('times', 'italic');
    ...
    // Bottom left: certificate number + stamp
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(9.5);
    ...
    doc.setFontSize(10);
    doc.setTextColor(40, 46, 58);
    doc.text(String(certificate.certCode || '').trim() || 'HB-XXXXXX', 78, 165);
    ...
    // Optional issued date (small, under signature line)
